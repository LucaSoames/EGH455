...
class GCSClient:
    ...
    def __init__(self, max_workers: int = 2):
        ...
        """
        # Point to backend API base (expects /api prefix)
        self.base_url = config.GCS_URL.rstrip('/') + "/api"
    ...
    def _send_post_request(self, url: str, **kwargs: Any) -> None:
        ...
        try:
            ...
            # Optional: attach bearer token if configured
            headers = kwargs.pop('headers', {}) or {}
            token = getattr(config, 'GCS_BEARER_TOKEN', None)
            if token:
                headers['Authorization'] = f"Bearer {token}"
            self.session.post(url, timeout=timeout, headers=headers, **kwargs)
