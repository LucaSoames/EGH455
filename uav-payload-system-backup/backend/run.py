...
from app.models import User, UAV, Mission, Payload, TelemetryData, Waypoint, SystemLog
...
def init_db():
    ...
    # Create sample payloads if not exist
    if Payload.query.count() == 0:
        sample_payloads = [
            Payload(
                name='HD Camera System',
                payload_type='camera',
                weight=0.8,
                dimensions='15 x 10 x 8 cm',
                description='High-definition camera for surveillance and mapping',
                status='available'
            ),
            Payload(
                ...
                description='FLIR thermal imaging system for search and rescue',
                status='available'
            ),
            Payload(
                name='Medical Supply Package',
                payload_type='medical_supplies',
                weight=2.0,
                dimensions='25 x 20 x 15 cm',
                description='Emergency medical supplies delivery package',
                status='available'
            ),
            Payload(
                name='Environmental Sensor Array',
                payload_type='environmental_sensor',
                weight=0.5,
                dimensions='12 x 8 x 6 cm',
                description='Multi-sensor array for environmental monitoring',
                status='available'
            )
        ]
        ...
        print(f"Created {len(sample_payloads)} sample payloads")
